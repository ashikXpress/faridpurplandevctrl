    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-default">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('area.update',['area'=>$area->id]) }}"
                    ...
                    <div class="card-body">
                        <div class="form-group row {{ $errors->has('area_no') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('area_no',$area->area_no) }}" name="area_no" class="form-control" id="area_no" placeholder="মহল্লা নং লিখুন">
                                @error('area_no')
                        ...
                        <div class="form-group row {{ $errors->has('area_name') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('area_name',$area->area_name) }}" name="area_name" class="form-control" id="area_name" placeholder="মহল্লা নাম লিখুন">
                                @error('area_name')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-sm-10">
                                <select name="ward" id="ward" class="form-control select2">
                                    ...
                                    @foreach($wards as $ward)
                                        <option {{ old('ward',$area->ward_id) == $ward->id ? 'selected' : '' }} value="{{ $ward->id }}">{{ $ward->ward_no }}</option>
                                    @endforeach
                                </select>
                                @error('ward')
                                <span class="help-block">{{ $message }}</span>
